...
** SVG with Config File (for Emacs text rendering)

Uses =:mermaid-config-file= to disable HTML labels, ensuring text renders in Emacs.

#+begin_src mermaid :file test-svg-emacs.svg :mermaid-config-file "mermaid-config-emacs.json"
flowchart LR
    A[Start] --> B[End]
#+end_src
